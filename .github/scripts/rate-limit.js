 ...
 * -----
 * Last Modified: Monday, 3rd November 2025 11:54:33 pm
...
async function checkForRecentSubmissions(owner, repo, issue) {
  ...
  console.log(`Oldest issue is #${oldestIssue.number}.`);

  const issuesToClose = sortedIssues.slice(1);

  for (const issueToClose of issuesToClose) {
    if (issueToClose.number === oldestIssue.number) continue;

    console.log(`Closing issue #${issueToClose.number} as a duplicate.`);
